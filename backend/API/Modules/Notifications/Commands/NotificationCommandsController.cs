...
public sealed class NotificationCommandsController : ControllerBase
{
    ...
    public NotificationCommandsController(ISender sender) => _sender = sender;

    [HttpPatch("{id:guid}/read")]
    public async Task<IActionResult> MarkAsRead(Guid id, CancellationToken cancellationToken = default)
        => await ExecuteAsync(
            userId => _sender.Send(new MarkNotificationAsReadCommand(userId, id), cancellationToken),
            ApiResponse<NotificationDto>.Error(401, "User not authenticated"));

    [HttpPatch("read-all")]
    public async Task<IActionResult> MarkAllAsRead(CancellationToken cancellationToken = default)
        => await ExecuteAsync(
            userId => _sender.Send(new MarkAllNotificationsAsReadCommand(userId), cancellationToken),
            ApiResponse<int>.Error(401, "User not authenticated"));
    ...
    public async Task<IActionResult> UpdateEmailPreference([FromBody] UpdateNotificationEmailPreferenceDto request, CancellationToken cancellationToken = default)
        => await ExecuteAsync(
            userId => _sender.Send(
                new UpdateNotificationEmailPreferenceCommand(
                    userId,
                    request.IsEmailEnabled,
                    request.IsTaskDeadlineReminderEmailEnabled),
                cancellationToken),
            ApiResponse<NotificationEmailPreferenceDto>.Error(401, "User not authenticated"));
