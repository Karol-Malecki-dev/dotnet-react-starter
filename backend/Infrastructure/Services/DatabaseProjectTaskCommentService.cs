...
public sealed class DatabaseProjectTaskCommentService : IProjectTaskCommentApplicationService
{
    ...
    public async Task<ProjectOperationResult<IReadOnlyList<ProjectTaskCommentView>>> GetProjectTaskCommentsAsync(Guid userId, Guid projectId, Guid taskId)
    {
    ...
    public async Task<ProjectOperationResult<ProjectTaskCommentView>> CreateProjectTaskCommentAsync(CreateProjectTaskCommentCommand command)
    {
        ...
        var role = await GetProjectRoleAsync(command.AuthorUserId, command.ProjectId);
        if (role is null || !await TaskBelongsToProjectAsync(command.ProjectId, command.ProjectTaskId))
        {
    ...
    public async Task<ProjectOperationResult<bool>> DeleteProjectTaskCommentAsync(Guid userId, Guid projectId, Guid taskId, Guid commentId)
    {
        ...
        var comment = await _dbContext.ProjectTaskComments
            .FirstOrDefaultAsync(candidate => candidate.Id == commentId && candidate.ProjectTaskId == taskId);
        if (comment is null)
        {
