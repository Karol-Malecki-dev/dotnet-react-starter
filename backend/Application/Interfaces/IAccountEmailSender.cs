...
/// <remarks>Implementations must pass the token to external delivery operations.</remarks>
public interface IAccountEmailSender
{
    ...
    Task SendEmailConfirmationAsync(string email, string displayName, string confirmationLink, CancellationToken cancellationToken = default);

    /// <summary>Sends a single-use password reset link.</summary>
    Task SendPasswordResetLinkAsync(string email, string displayName, string resetLink, CancellationToken cancellationToken = default);
    ...
    Task SendTwoFactorCodeAsync(string email, string displayName, string code, DateTime expiresAt, CancellationToken cancellationToken = default);
}